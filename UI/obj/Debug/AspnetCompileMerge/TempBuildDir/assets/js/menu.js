; $(function () {
    ...
    $.post("/wxmenu/getmenu", { token: store.get("token") }, function (res) {
        if (res.code === 0) {
            obj = res.data;
            ...
            menu = '<div class="custom-menu-view__menu"><div class="text-ellipsis"></div></div>';
            customBtns = $('.custom-menu-view__footer__right');
            if (button.length > 0) {
                showMenu();
                //$('.cm-edit-before').hide();
                $('.cm-edit-after').hide();
    ...
    function showBtn() {
        for (var i = 0; i < button.length; i++) {
            ...
            if (button[i].sub_button) {
                ...
                ulBtnL = button[i].sub_button.length;
            }
    ...
    function setLiId() {
        ...
        if (!button[colIndex].sub_button) {
            button[colIndex].sub_button = [];
        }
    ...
    function saveAjax() {
        ...
        var newJson = {};//去除delete的菜单数组数据
        newJson.menu = {};
        if (obj.menu.button) {
            newJson.menu.button = obj.menu.button.filter(res => { return res });
            for (var i = 0; i < newJson.menu.button.length; i++) {
                if (newJson.menu.button[i].sub_button != undefined) {
                    var newButton = newJson.menu.button[i].sub_button.filter(res => { return res });
                    newJson.menu.button[i].sub_button = newButton;
                }
            }
        }

        $.post("/wxmenu/CreateMenuFromJson", { fullJson: JSON.stringify(newJson), token: store.get("token") }, function (res) {
            layui.layer.msg(res.msg);
        });
    }
});