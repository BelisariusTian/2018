...
namespace Common
{
    public class ImageClass
    {
		...
		public static string Generate2Code2(string path, string str) {
			...
			//导出图片  
			Bitmap p = writer.Write(str);
			p.Save(path, System.Drawing.Imaging.ImageFormat.Png);
			p.Dispose();
			//在程序中加载导出的二维码  
			return path;
		}
		...
		public static string DowloadImgByUrl(string path,string savePath) {
			try
			{
				System.Net.WebClient m_hvtWebClient = new System.Net.WebClient();

				//生成随机的图片文件名
				string m_picFileName = Guid.NewGuid().ToString();
				string m_keleyiPicture = savePath + m_picFileName;
				m_hvtWebClient.DownloadFile(path, m_keleyiPicture);
				return m_keleyiPicture;
			}
			catch (Exception)
			{
